...
class UploadManagerTest(BaseEntitlementTestCase):
    ...
    def setUp(self):
        ...
        self.mocked_arg_of_pu_data = None
        def mocked_upload_product_usage_data(addr, port, url, pu_data):
            self.mocked_arg_of_pu_data = pu_data
        self.orig_upload_product_usage_data = splice_server_client.upload_product_usage_data
        splice_server_client.upload_product_usage_data = mocked_upload_product_usage_data
...
""" % (servers)
    ...
    def test_upload_product_usage_data_with_bad_config(self):
        servers = "255.255.255.255:443:/splice/api/v1/productusage,127.0.0.1:80:/foo/bar/bz,bad_value_missing_end_colon_before_url:80/foo/bar/bz2"
    ...
    def test_upload_product_usage_data(self):
        servers = "255.255.255.255:443:/splice/api/v1/productusage,127.0.0.1:80:/foo/bar/bz,127.0.0.1:80:/foo/bar/bz2"
        called_args = []
        ...
        for arg in called_args:
            ...
            self.assertTrue(arg[2] in ["/splice/api/v1/productusage", "/foo/bar/bz", "/foo/bar/bz2"])
    ...
    def test_process_product_usage_upload(self):
        ...
        self.assertEqual(len(self.mocked_arg_of_pu_data), 2)
        ...
        # Simulate next invocation
        upload._process_product_usage_upload(addr, port, url, limit)
        self.assertEqual(len(self.mocked_arg_of_pu_data), 1)
    ...
    def test_update_last_timestamp_empty(self):
        ...
        upload._update_last_timestamp(addr, timestamp)
